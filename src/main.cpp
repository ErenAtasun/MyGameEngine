...
// Mouse tekeri için controller'a eriþim
...
int main() {
...
#ifdef _WIN32
    // Çalýþma dizinini exe klasörüne çek (assets eriþimi için)
    ...
    GLuint texAtlas = std::filesystem::exists("assets/atlas.png") ? Renderer2D::LoadTexture("atlas.png") : tex1;
    ...
    float animTime = 0.0f;   // saniye
    float animFps = 8.0f;   // saniyede 8 kare
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        // Resize & kamera güncelle
        glfwGetFramebufferSize(win, &fbw, &fbh);
        camCtrl.OnResize((float)fbw, (float)fbh);
        camCtrl.OnUpdate(dt, win);          // WASD hareket
        // Mouse wheel scroll_cb ? camCtrl.OnScroll

        // Animasyon kare ilerletme
        animTime += dt;
        ...
        // Örnek: ok tuþlarýyla tek bir sprite'ý da kaydýr
